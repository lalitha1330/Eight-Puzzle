...
public class Puzzle {
	...
	// defining the goalState as a 2-Dimensional 3×3 matrix 
	public static final int[][] goalState=new int[3][3] ;
	public static void main(String[] args) {
		Nodes main= new Nodes();
		goalState[0][0]=1;
		goalState[0][1]=2;
		goalState[0][2]=3;
		goalState[1][0]=4;
		...
		while(true){
			...
			int puzzle_no= scanner.nextInt();
			
	//User can enter only 1 and 2 for using the puzzle solver
			while(puzzle_no<1 && puzzle_no>3 ){ 
				System.out.println("Enter the value for using the puzzle solver");
				System.out.println("Type 1 to use a default puzzle, or 2 to enter your own puzzle or 3 to exit.");
				puzzle_no= scanner.nextInt();
			}
			if(puzzle_no==1){
				
    //Using the default puzzle
				initialState[0][0]=1;
				initialState[0][1]=8;
				initialState[0][2]=2;
				initialState[1][0]=0;
	...
	// own puzzle
				for(i=0;i<3;i++){
					System.out.println("Enter "+(i+1)+"st row space or tabs between numbers");
					for(j=0;j<3;j++)
	...
	//verifying whether the entered matrix has elements from 0-8
